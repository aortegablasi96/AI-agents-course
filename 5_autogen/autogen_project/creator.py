...
class Creator(RoutedAgent):
    ...
    You can choose to take this Agent in a completely different direction. The only requirement is that the class must be named as given by the user,
    ...
    async def handle_my_message_type(self, message: messages.Message, ctx: MessageContext) -> messages.Message:
        ...
        agent_type = "agent" if "agent" in agent_name else "creator"

        text_message = TextMessage(content=self.get_user_prompt(agent_type), source="user")
        ...
        if agent_type == "creator":
            ...
            return messages.Message(content=agent_name)
